...
A 2D implementation of the Lattice-Boltzmann Method
...
def even_print( label , output , **kwargs ):
    ...
    #sys.stdout.write(text) #print directly
    print(text)
    #return text
    return
...
def equilibrium(rho, ux, uy):
    '''
    get equilibrium distribution from macroscopic variables
    '''
    ux2   = ux**2
    uy2   = uy**2
    umag2 = ux2+uy2
...
class lbm(object):
    ...
    '''
    def __init__(self, Re=1000, M_inf=0.05, fn_h5='data.h5', nx=1920, ny=1080, **kwargs):
        ...
        if self.start_from_restart:
            ...
            self.l_lattice = self.lchar
            
            ## ν = Δt·cs²·(τ-(1/2))
            self.nu_lattice = self.u_lattice * self.l_lattice / self.Re
        ...
        even_print('M_inf'  , '%0.6f [-]' % M_inf  )
        even_print('nx'     , '%i'        % nx     )
        even_print('ny'     , '%i'        % ny     )
        even_print('data save period', '%i [ts]'%self.data_save_period )
        even_print('u_lattice', '%0.6f' % self.u_lattice )
    ...
    def load_restart(self,fn_restart_dat):
        ...
        self.l_lattice  = self2.l_lattice
    ...
    def set_obstacle(self,):
        ...
        if (self.geom_preset=='cylinder') or (self.geom_preset=='circle'):
            ...
            #self.radius   = int(round(0.05*self.height))+0.5 
            self.radius   = int(round(0.075*self.height))+0.5
            #self.radius   = int(round(0.05*1080))+0.5
            
            self.cx       = int(round(0.20*self.width))
            #self.cx       = int(round(0.33*1920))
    ...
    def initialize_flowfield(self,):
        '''
        set the initual flow field [u,v,ρ]
        '''
    ...
    def set_timing(self,):
        ...
        #self.pic_output_period = 200
        #self.pic_output_period = 400
        pass
        return
    ...
    def initialize_h5(self,):
        ...
        with h5py.File(self.fn_h5, 'w', libver='latest') as hf:
            ...
            hf.create_dataset('U_inf'    , data=(self.u_lattice,)  , maxshape=(1,))
            hf.create_dataset('Re'       , data=(self.Re,)         , maxshape=(1,))
            hf.create_dataset('nu_inf'   , data=(self.nu_lattice,) , maxshape=(1,))
            hf.create_dataset('lchar'    , data=(self.l_lattice,)  , maxshape=(1,))
    ...
    def run(self,nts):
        ...
        #even_print('output period', '%i [ts]'%self.pic_output_period)
        even_print('data save period', '%i [ts]'%self.data_save_period )
        
        ## u,v,ρ single precision
        data_b_per_period = 3*4*nx*ny
        even_print('data per period', '%0.1f [MB]'%(data_b_per_period/1024**2,))
        
        periods_this_run = math.floor(nts/self.data_save_period)
        even_print('n periods', '%i'%(periods_this_run,))
        
        data_gb_this_run = data_b_per_period * periods_this_run / 1024**3
...
if __name__ == '__main__':
    ...
    #nx = int(round(1920*1)) + 1500
    #ny = int(round(1080*1)) + 500 + 500
    #nx, ny = 800, 300
    nx, ny = 1280, 720
    ...
    sim = lbm(Re=1000, M_inf=0.05,
              fn_h5='data.h5',
              nx=nx, ny=ny, 
              geom_preset='cylinder',
              max_time_s=23*3600,
              pic_output_period=200,
              bcNS='slip_wall',
              restart=restart)
    sim.run(nts=100_000)
    sim.encode_video(fps=60)
